...
function Layout() {
  ...
  const fetchAndSetCourses = async () => {
    try {
      const response = await axios.get("http://localhost:3003/api/courses");
  ...
  const fetchAndSetStudents = async () => {
    try {
      const response = await axios.get("http://localhost:3003/api/students");
      const data = response.data;
  ...
  return (
    <Container style={{ marginLeft: "6vw", marginRight: "5vw", marginTop: "1vw" }}>
      <Grid
        container
        spacing={3}
        style={{
